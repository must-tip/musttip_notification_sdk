...
export class NotificationsClient {
    ...
    createNotification(notification, idempotencyKey = `notification-${crypto.randomUUID()}`) {
    ...
    buildUrl(template, pathValues, query) {
        ...
        for (const name of required) {
            if (!(name in pathValues))
                throw new ProtocolError(`missing path parameter ${name}`);
        ...
        for (const [key, raw] of Object.entries(query)) {
            ...
            for (const value of values)
                url.searchParams.append(key, typeof value === "boolean" ? String(value).toLowerCase() : String(value));
        }
        return url.toString();
    }
