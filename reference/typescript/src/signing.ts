...
export async function signEventPayload(payload: Uint8Array, secret: string, keyId: string): Promise<Record<string, string>> {
...
export async function verifyEventPayload(
  payload: Uint8Array,
  ...
  if (!signature && !keyId) {
    if (required) throw new ProtocolError("event signature is required");
    return false;
  }
